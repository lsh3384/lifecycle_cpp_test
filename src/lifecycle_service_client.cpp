...
void
custom_change_state(std::shared_ptr<LifecycleServiceClient> lc_client,
uint8_t transition_type) {
  {
    if (!rclcpp::ok()) {
      return;
    }
    if (!lc_client->change_state(transition_type)) {
      return;
    }
...
int main(int argc, char ** argv)
{
  ...
  while (1) {
    ...
    command = "";
    std::cin >> command;
    std::chrono::duration<int64_t, std::milli> timeout = std::chrono::duration<int64_t, std::milli>(1000);

    // std::cout << command;
    if (command == "configure") {
      std::shared_future<void> script = std::async(
      std::launch::async,
      std::bind(custom_change_state, lc_client, lifecycle_msgs::msg::Transition::TRANSITION_CONFIGURE));
      exe.spin_until_future_complete(script, timeout);
      std::cout << "spin_until_future_complete finished";

    } else if (command == "activate") {
      std::shared_future<void> script = std::async(
      std::launch::async,
      std::bind(custom_change_state, lc_client, lifecycle_msgs::msg::Transition::TRANSITION_ACTIVATE));
      exe.spin_until_future_complete(script);
      std::cout << "spin_until_future_complete finished";
    } else if (command == "deactivate") {
      std::shared_future<void> script = std::async(
      std::launch::async,
      std::bind(custom_change_state, lc_client, lifecycle_msgs::msg::Transition::TRANSITION_DEACTIVATE));
      exe.spin_until_future_complete(script);
      std::cout << "spin_until_future_complete finished";
